...
{
    _NetworkStatusDNS _current;
    _NetworkStatusDNS _last;
...
- (_NetworkStatusDNS)reachabilityFlags:(SCNetworkReachabilityFlags)flags
{
    if ((flags & kSCNetworkReachabilityFlagsReachable) == 0 || ![self internetConnection]) {
        // The target host is not reachable.
        return NotReachableDNS;
    }
    
    _NetworkStatusDNS returnValue = NotReachableDNS;
    if ((flags & kSCNetworkReachabilityFlagsConnectionRequired) == 0) {
        returnValue = ReachableViaWiFiDNS;
    }
    ...
    if ((flags & kSCNetworkReachabilityFlagsIsWWAN) == kSCNetworkReachabilityFlagsIsWWAN) {
        returnValue = ReachableVia4GDNS;
    }
    
    if ((flags & kSCNetworkReachabilityFlagsIsWWAN) == kSCNetworkReachabilityFlagsIsWWAN) {
        if((flags & kSCNetworkReachabilityFlagsReachable) == kSCNetworkReachabilityFlagsReachable) {
            if ((flags & kSCNetworkReachabilityFlagsTransientConnection) == kSCNetworkReachabilityFlagsTransientConnection) {
                returnValue = ReachableVia3GDNS;
                
                if((flags & kSCNetworkReachabilityFlagsConnectionRequired) == kSCNetworkReachabilityFlagsConnectionRequired) {
                    returnValue = ReachableVia2GDNS;
                }
            }
        }
    }
...
-(void)update {
    ...
    if (SCNetworkReachabilityGetFlags(_ref, &flags)) {
        ...
        // change bssid
        if (_current == ReachableViaWiFiDNS) {
